...
class Settings(BaseSettings):
    ...
    service_name: str = Field(default="recovery-case-service", alias="SERVICE_NAME")
    ...
    database_url: str = Field(
        default="postgresql+psycopg://postgres:postgres@localhost:5436/recovery_case_service",
        alias="DATABASE_URL",
    )
    ...
    rabbitmq_match_events_queue: str = Field(
        default="recovery-case-service.match-events",
        alias="RABBITMQ_MATCH_EVENTS_QUEUE",
    )
    item_service_base_url: str = Field(
        default="http://localhost:8002",
        alias="ITEM_SERVICE_BASE_URL",
    )
    item_service_timeout_seconds: float = Field(
        default=10.0,
        alias="ITEM_SERVICE_TIMEOUT_SECONDS",
    )
