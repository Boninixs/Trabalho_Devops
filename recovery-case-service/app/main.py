...
from app.messaging.consumer import MatchAcceptedConsumer
...
async def lifespan(_: FastAPI):
    ...
    if settings.event_consumer_enabled:
        await match_accepted_consumer.start()
    else:
        logger.info("match_accepted_consumer_disabled")
    yield
    await outbox_publisher.stop()
    await match_accepted_consumer.stop()
